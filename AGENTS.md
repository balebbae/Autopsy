...
- **Pre-flight context injection** lives in `plugin/src/handlers/tool-before.ts`. For tools in `AAG_PREFLIGHT_TOOLS` (default: `edit,write,bash,read,grep`) it calls `POST /v1/preflight`, throws a graph-cited rationale on `block: true` (emitting `aag.preflight.blocked`), and emits `aag.preflight.warned` for non-blocking risk. Bounded by `AAG_PREFLIGHT_TIMEOUT_MS` (default 800ms) with fail-open. Smoke test: `cd plugin && bun src/__smoke__/tool-before.smoke.ts`. Knobs: `AAG_PREFLIGHT_DISABLED`, `AAG_PREFLIGHT_TIMEOUT_MS`, `AAG_PREFLIGHT_TOOLS`.
